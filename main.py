...
class JP2Viewer:
    ...
    def start_processings(self):
        self.producer = multiprocessing.Process(
            target=JP2Viewer.producer_task_static,
            args=(self.files, self.task_queue, self.num_consumers, self.stop_event)
        )
        self.producer.start()
    ...
    def check_loading(self):
        ...
        if first_frame is not None:
            ...
            self.info.config(text=f"Загрузка первого кадра({loaded}/{self.number_of_frames})")
